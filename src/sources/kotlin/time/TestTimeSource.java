...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1541plusAssignLRDsOJo(long duration) {
        long longDelta = Duration.m1451toLongimpl(duration, getUnit());
        ...
        if ($i$f$isSaturated == 0) {
            ...
            if ((j ^ longDelta) >= 0 && (j ^ newReading) < 0) {
                m1540overflowLRDsOJo(duration);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m1408divUwyO8pc(duration, 2);
        long $this$isSaturated$iv = Duration.m1451toLongimpl(half, getUnit());
        if (!((($this$isSaturated$iv - 1) | 1) == Long.MAX_VALUE)) {
            long readingBefore = this.reading;
            try {
                m1541plusAssignLRDsOJo(half);
                m1541plusAssignLRDsOJo(Duration.m1440minusLRDsOJo(duration, half));
                return;
    ...
    private final void m1540overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m1454toStringimpl(duration)) + '.');
    }
}
